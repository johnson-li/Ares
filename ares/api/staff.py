...
@restful_request
@login_required
def staff_query(user_id, offset=0, page_size=10, orders='', name='', company='', registrationId='', title=''):
    where = []
    if name:
    ...
    cursor = get_cursor()
    sql = 'select * from Staff {} {} limit {} offset {}'.format(where, order, page_size, offset)
    print sql
    cursor.execute(sql)
